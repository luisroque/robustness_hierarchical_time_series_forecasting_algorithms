...
class PreprocessDatasetsTestCase(unittest.TestCase):
    def setUp(self):
        self.preprocess_datasets = PreprocessDatasets(dataset="synthetic", freq="W")

    def test_add_synthetic_dataset(self):
        self.preprocess_datasets._synthetic()

        groups = self.preprocess_datasets._load_pickle_file(self.preprocess_datasets.pickle_path)
